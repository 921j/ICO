	...
	Else
	{
      ...
      Loop %LastIdx%
      {
        ...
        If (oAcc.accName(A_Index) = "√")
          ;Acc_Get("DoAction", "4.3.4.1.4.1.4.1.4.1.4.1.4", A_Index, "ahk_id" hWnd)
          CurrIdx := A_Index
      ...
      if (maxi != CurrIdx)
      {
        ...
        send {enter}
        MsgBox, 4, ,  v2rayN 已选最优节点.`n点击确定关闭程序界面., 8
          IfMsgBox Yes
            WinHide, % v2rayNCls
      }
			else
      {
        MsgBox, 4, ,  v2rayN 已是最优节点.`n点击确定关闭程序界面., 8
          IfMsgBox Yes
            WinHide, % v2rayNCls
			}
			oAcc := ""
